...
def qwen_summarize(
    ...
    - a punchy English summary
    - a punchy Hindi summary
    - up to 4 source-supported image subjects
    """
    ...
    # Generated by qwen_converter.properties and consumed by the
    # Wikipedia/Wikimedia image fetcher.
    raw_image_subjects = data.get("image_subjects", [])
    if isinstance(raw_image_subjects, str):
        raw_image_subjects = [raw_image_subjects]
    if not isinstance(raw_image_subjects, list):
        raw_image_subjects = []

    blocked_image_subjects = {
        "box office", "blockbuster", "hollywood", "bollywood",
        "movie business", "entertainment", "cinema", "film industry",
        "movie industry", "indian cinema",
    }

    image_subjects = []
    for subject in raw_image_subjects:
        subject = clean_text(subject)
        if not subject or subject.lower() in blocked_image_subjects:
            continue
        if subject not in image_subjects:
            image_subjects.append(subject)
        if len(image_subjects) >= 4:
            break
